...
Scenario: BU - As user with edit selected columns with editable row filter and view selected columns with viewable row filter, using parameters, Insert into a Link Column using ResolveLink with no permission to that column
	When As User ViewSelected EditSelected View Edit RF Run Bulk Upsert cql statement INSERT INTO [Mock].[BulkUpsertNegative1] ([Cinchy Id],[Person]) VALUES (@data0, ResolveLink(@data1,'Name')); with Parameter:Column:Type as data0:Cinchy Id:Number,data1:Person:Text,data2:Link Calculated Cached:Text
...
Scenario: BU - As user with edit selected columns with editable row filter and view selected columns with viewable row filter, Insert into a Multi Select Link Column with no permission to that column
	When As User ViewSelected EditSelected View Edit RF Run Bulk Upsert cql statement INSERT INTO [Mock].[BulkUpsertNegative1] ([Cinchy Id],[Name],[MSL]) VALUES (@data0, @data1, ResolveLink(@data2,'Name')); with Parameter:Column:Type as data0:Cinchy Id:Number,data1:Name:Text,data2:MSL:Text
		| Value0 | Value1 | Value2        |
...
Scenario: BU - As user with edit selected columns with editable row filter and view selected columns with viewable row filter, Insert into a Multi Select Choice Column with no permission to that column
	When As User ViewSelected EditSelected View Edit RF Run Bulk Upsert cql statement INSERT INTO[Mock].[BulkUpsertNegative1] ([Cinchy Id],[Name],[MSC]) VALUES(@data0, @data1, @data2) with Parameter:Column:Type as data0:Cinchy Id:Number,data1:Name:Text,data2:MSC:Text
		| Value0 | Value1 | Value2           |
...
Scenario: BU - As user with edit selected columns with editable row filter and view selected columns with viewable row filter, Insert into hierarchy link column using ResolveLink with no permission to that column
	When As User ViewSelected EditSelected View Edit RF Run Bulk Upsert cql statement INSERT INTO [Mock].[BulkUpsertNegative1] ([Cinchy Id],[HLink]) VALUES (@data0, ResolveLink(@data1,'Name')); with Parameter:Column:Type as data0:Cinchy Id:Number,data1:HLink:Text
		| Value0 | Value1 |
		| 9      | Annie  |
		| 10     | Mark   |
		|        | Annie  |
...
Scenario: BU - As user with edit selected columns with editable row filter and view selected columns with viewable row filter, Insert into multi select hierarchy link column using ResolveLink
	When As User ViewSelected EditSelected View Edit RF Run Bulk Upsert cql statement INSERT INTO [Mock].[BulkUpsertNegative1] ([Cinchy Id],[MSH]) VALUES (@data0, ResolveLink(@data1,'Name')); with Parameter:Column:Type as data0:Cinchy Id:Number,data1:MSH:Text
		| Value0 | Value1 |
		| 9      | Annie  |
		| 10     | Mark   |
		|        | Annie  |
...
Scenario: BU - As user with edit selected columns with editable row filter and view selected columns with viewable row filter, Insert into live and Cached Calculated columns tha references simple column types, UDFs
	When As User ViewSelected EditSelected View Edit RF Run Bulk Upsert cql statement INSERT INTO [Mock].[BulkUpsertNegative1] ([Cinchy Id], [Calculated Cache], [Calculated Live], [Live UDF],[Cached UDF], [Name]) VALUES (@data0,@data1,@data2,@data3,@data4,@data5) with Parameter:Column:Type as data0:Cinchy Id:Number,data1:Calculated Cache:Text,data2:Calculated Live:Text,data3:Live UDF:Text,data4:Cached UDF UDF:Text,data5:Name:Text
		| Value0 | Value1      | Value2 | Value3    | Value4 | Value5  |
		| 9      | 15 Mar 2020 | 66000  | Co-op     | Yes    | Annie   |
		| 10     | Dec 2 2020  | 67000  | Full Time | No     | Mark    |
		|        | Dec 3 2020  | 68000  | Part Time | Yes    | Apricot |
...
Scenario: BU - As user with edit selected columns with editable row filter and view selected columns with viewable row filter, Insert into a Link Column that references cached calculated column type
	When As User ViewSelected EditSelected View Edit RF Run Bulk Upsert cql statement INSERT INTO [Mock].[BulkUpsertNegative1] ([Cinchy Id],[Link Calculated Cache]) VALUES (@data0, ResolveLink(@data1,'Calculated Cache')); with Parameter:Column:Type as data0:Cinchy Id:Number,data1:Link Calculated Cached:Text
		| Value0 | Value1                         |
